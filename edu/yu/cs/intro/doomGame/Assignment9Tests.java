...
public class Assignment9Tests {
    ...
    public void runTests() {
        // tests on Monster
        this.runTest("testAttackWithInsufficientWeapon",
                "Monster: does monster.attack throw an IllegalArgumentException when attacked with an insufficient weapon?");
        this.runTest("testAttackWithNoAmmo",
                "Monster: does monster.attack throw an IllegalArgumentException when attacked with no ammunition?");
        this.runTest("testAttackDeadMonster",
                "Monster: does Monster.attack throw an IllegalStateException when the monster was attacked when it was already dead?");
        this.runTest("testAttackMonsterButDontKillMonster",
                "Monster: Monster.isDead should return false after being attacked with less than ammunitionCountNeededToKill");
        this.runTest("testKillMonsterInTwoPasses",
                "Monster: Monster attacked with ammunitionCountNeededToKill over the course of 2 calls on monster.attack should be dead, i.e. should return true from isDead()");
        this.runTest("testGetDefaultProtector",
                "Monster: Monster with no custom protector should return the protector of its monster type from Monster.getProtectedBy()");
        this.runTest("testGetCustomProtector",
                "Monster: Monster with custom protector should return the custom protector from Monster.getProtectedBy()");
        ...
        this.runTest("testChangePlayerHealth",
                "Player: Does Player.changeHealth result in health changing as expected?");
        this.runTest("testReducePlayerHealth",
                "Player: Does Player.changeHealth, with a negative number, result in health changing as expected?");
        this.runTest("testAddAmmo",
                "Player: Does Player.addAmmunition result in ammunition count changing as expected?");
        this.runTest("testChangeAmmo",
                "Player: Does Player.changeAmmunitionRoundsForWeapon result in ammunition count changing as expected?");
        this.runTest("testReduceAmmo",
                "Player: Does Player.changeAmmunitionRoundsForWeapon result in ammunition count beign reduced as expected?");
        this.runTest("testHasWeapon",
                "Player: Does Player.hasWeapon return what is expected based on what weapons we have given him?");
        this.runTest("testPlayerCompareTo", "Player: Does Player.compareTo return what is expected?");

        // tests on Room
        this.runTest("testRoomDangerLevel",
                "Room: Danger level should equal ordinal+1 values of all monsters combined");
        this.runTest("testRoomCompareTo", "Room: compareTo should compare based on danger level");
        this.runTest("testGetWeaponsWonUponCompletion",
                "Room: did not get expected set of weapons won upon completion");
        this.runTest("testGetWeaponsWonUponCompletion",
                "Room: check keys and values of room.getAmmoWonUponCompletion()");
        ...
        this.runTest(36, "testReapCompletionRewards",
                "GameBot: Check that player reaps the rewards of completing a room: GameBot.reapCompletionRewards()");
        this.runTest(36, "testKillMonster",
                "GameBot: Check that killMonster kills the right set of monsters and results in the correct changes to the player");
        this.runTest("testGetCompletedRooms",
                "GameBot: Check that Gamebot.getCompletedRooms() returns all completed rooms, and no other rooms");
        this.runTest("testGetLivePlayers",
                "GameBot: Check that Gamebot.getLivePlayers() returns all live players, and no other players");
        this.runTest("testGetLivePlayersWithWeaponAndAmmunition",
                "GameBot: Check that Gamebot.getLivePlayersWithWeaponAndAmmunition() returns the correct set of players");
        this.runTest(24, "testGetAllProtectorsInRoom",
                "GameBot: Check that Gamebot.getAllProtectorsInRoom() returns the correct set of monsters");
    ...
    public void testCanKill() {
        ...
        if (GameBot.canKill(p, demon, fearsome)) {
            throw new IncorrectBehaviorException(
                    "GameBot.canKill returned true for a player with the chainsaw and 5 rounds of ammo being able to kill the demon even through he doesn't have the shotgun to kill his protector, the baron of hell");
        }
        p.addWeapon(Weapon.SHOTGUN);
        p.addAmmunition(Weapon.SHOTGUN, 7);
        if (GameBot.canKill(p, demon, fearsome)) {
            throw new IncorrectBehaviorException(
                    "GameBot.canKill returned true for a player being able to kill a demon even though he doesn't have the pistol to kill the protector's protector, i.e. spectre");
        }
        p.addWeapon(Weapon.PISTOL);
        if (GameBot.canKill(p, demon, fearsome)) {
            throw new IncorrectBehaviorException(
                    "GameBot.canKill returned true for a player being able to kill a demon even though he doesn't have enough pistol ammo to kill the protector's protector, i.e. spectre");
        }
        p.addAmmunition(Weapon.PISTOL, 1);
        if (!GameBot.canKill(p, demon, fearsome)) {
            throw new IncorrectBehaviorException(
                    "GameBot.canKill returned false for a player being able to kill a demon even though he should be able to");
        }
        p.setHealth(10);
        if (GameBot.canKill(p, demon, fearsome)) {
            throw new IncorrectBehaviorException(
                    "GameBot.canKill returned true for a player being able to kill a demon even though he will run out of health before killing the demon");
        }
    }
    ...
    public void testGetAllProtectorsInRoom() {
        ...
        if (protectors.size() != 1 || !protectors.contains(spectre)) {
            throw new IncorrectBehaviorException(
                    "GameBot.getAllProtectorsInRoom did not return the one protector of the baron of hell");
        }
    }
    ...
    public void testGetLivePlayersWithWeaponAndAmmunition() {
        ...
        if (armed.size() != 1 || !armed.contains(p2)) {
            throw new IncorrectBehaviorException(
                    "bot.getLivePlayersWithWeaponAndAmmunition did not return the expected player (p2)");
        }
        ...
        if (armed.size() != 2 || !armed.contains(p2) || !armed.contains(p1)) {
            throw new IncorrectBehaviorException(
                    "bot.getLivePlayersWithWeaponAndAmmunition did not return the expected players (p1,p2)");
        }
        ...
        if (armed.size() != 1 || !armed.contains(p4)) {
            throw new IncorrectBehaviorException(
                    "bot.getLivePlayersWithWeaponAndAmmunition did not return the expected players (p4) CUSTOM INFO:  "
                            + armed.contains(p4) + p4.hasWeapon(Weapon.CHAINSAW)
                            + p4.getAmmunitionRoundsForWeapon(Weapon.CHAINSAW));
        }
        // NO player with the weapon & ammo
        armed = bot.getLivePlayersWithWeaponAndAmmunition(Weapon.SHOTGUN, 100);
        if (armed.size() != 0) {
            throw new IncorrectBehaviorException(
                    "bot.getLivePlayersWithWeaponAndAmmunition did not return the expected players (none!)");
        }
        // ALL players with the weapon & ammo
        armed = bot.getLivePlayersWithWeaponAndAmmunition(Weapon.FIST, 1000);
        if (armed.size() != 4) {
            throw new IncorrectBehaviorException(
                    "bot.getLivePlayersWithWeaponAndAmmunition did not return the expected players (all players)");
        }
    }
    ...
    public void testGetLivePlayers() {
        ...
        if (live.size() != 2 || !live.contains(p1) || !live.contains(p3)) {
            throw new IncorrectBehaviorException(
                    "A Player which was alive was missing in the set returned by GameBot.getLivePlayers() or there was the wrong number elements in the set");
        }
    }
    ...
    public void testGetCompletedRooms() {
        ...
        if (rooms.size() != 2 || findRoomByName(rooms, "Easy Imps") == null
                || findRoomByName(rooms, "Double Demonic") == null) {
            throw new IncorrectBehaviorException(
                    "A Room which was completed was missing in the set returned by GameBot.getCompletedRooms() or there was the wrong number elements in the set");
        }
    }
    ...
    public void testKillMonster() {
        ...
        if (demon.getMonsterType().ammunitionCountNeededToKill > 5) {
            p.addAmmunition(demon.getMonsterType().weaponNeededToKill,
                    demon.getMonsterType().ammunitionCountNeededToKill - 5);
        }
        p.addWeapon(baron.getMonsterType().weaponNeededToKill);
        if (baron.getMonsterType().ammunitionCountNeededToKill > 5) {
            p.addAmmunition(baron.getMonsterType().weaponNeededToKill,
                    baron.getMonsterType().ammunitionCountNeededToKill - 5);
        }
        players.add(p);
        ...
        if (!baron.isDead()) {
            throw new IncorrectBehaviorException(
                    "after killMonster was called on demon, baronOfHell should be dead as well");
        }
        if (!demon.isDead()) {
        ...
        if (p.getAmmunitionRoundsForWeapon(baron.getMonsterType().weaponNeededToKill) != 0) {
            throw new IncorrectBehaviorException(
                    "killing the baron of hell should've used up all the ammunition for the weapon needed to kill it");
        }
        if (p.getAmmunitionRoundsForWeapon(demon.getMonsterType().weaponNeededToKill) != 4) {
            throw new IncorrectBehaviorException(
                    "killing the demon should've used up one round of chainsaw ammo, leaving 4");
        }
        // check that health has been reduced
        int expectedReduction = (demon.getMonsterType().playerHealthLostPerExposure * 2)
                + baron.getMonsterType().playerHealthLostPerExposure
                + (imp.getMonsterType().playerHealthLostPerExposure * 2);
        if (p.getHealth() != initialhealth - expectedReduction) {
            throw new IncorrectBehaviorException(
                    "Player health was nto reduced by expected amount when killing a monster");
        }
    }
    ...
    public void testMultiPlayerFailureGame() {
        ...
        if (success) {
            throw new IncorrectBehaviorException(
                    "The test game " + gameName + " succeeded, although it was expected to fail");
        }
    }
    ...
    public void testMultiPlayerSuccessGame() {
        ...
        if (!success) {
            throw new IncorrectBehaviorException(
                    "The test game " + gameName + " failed, although it was expected to succeed");
        }
    }
    ...
    public void testGetPlayerHealthLostPerEncounter() {
        ...
        if (healthLost != room.getPlayerHealthLostPerEncounter()) {
            throw new IncorrectBehaviorException(
                    "room.getPlayerHealthLostPerEncounter() should equal the sum of playerHealthLostPerExposure values of all live monsters combined. Expected: "
                            + healthLost + ". Actual: " + room.getPlayerHealthLostPerEncounter());
        }
    }
    ...
    public void testGetAmmoWonUponCompletion() {
        ...
        Map<Weapon, Integer> ammoWonUponCompletion = new HashMap<>();
        Room room = new Room(monsters, weapons, ammoWonUponCompletion, 20, "Fearsome Foursome");
        if (!room.getWeaponsWonUponCompletion().contains(Weapon.CHAINSAW)
                || !room.getWeaponsWonUponCompletion().contains(Weapon.PISTOL)
                || room.getWeaponsWonUponCompletion().size() != 2) {
    ...
    public void testGetWeaponsWonUponCompletion() {
        ...
        ammoWonUponCompletion.put(Weapon.CHAINSAW, 10);
        Room room = new Room(monsters, weapons, ammoWonUponCompletion, 20, "Fearsome Foursome");
        if (!room.getAmmoWonUponCompletion().keySet().contains(Weapon.PISTOL)
                || !room.getAmmoWonUponCompletion().keySet().contains(Weapon.CHAINSAW)) {
            throw new IncorrectBehaviorException(
                    "An expected key is missing from the set returned by room.getAmmoWonUponCompletion()");
        }
        if (room.getAmmoWonUponCompletion().get(Weapon.PISTOL) != 5
                || room.getAmmoWonUponCompletion().get(Weapon.CHAINSAW) != 10) {
            throw new IncorrectBehaviorException(
                    "An expected value is wrong in the set returned by room.getAmmoWonUponCompletion()");
        }
    }
    ...
    public void testRoomCompareTo() {
        ...
        if (room1.compareTo(room2) != -1) {
            throw new IncorrectBehaviorException(
                    "compareTo should compare based on danger level; expected -1, actual return value: "
    ...
    public void testRoomDangerLevel() {
        ...
        if (danger != room.getDangerLevel()) {
            throw new IncorrectBehaviorException(
                    "Danger level should equal ordinal+1 values of all live monsters combined. Expected: " + danger
                            + ". Actual: " + room.getDangerLevel());
        }
        baron.attack(baron.getMonsterType().weaponNeededToKill, baron.getMonsterType().ammunitionCountNeededToKill);
        room.monsterKilled(baron);
        spectre.attack(spectre.getMonsterType().weaponNeededToKill,
                spectre.getMonsterType().ammunitionCountNeededToKill);
    ...
    public void testHasWeapon() {
        ...
        if (!(p.hasWeapon(Weapon.CHAINSAW) && p.hasWeapon(Weapon.FIST))) {
            throw new IncorrectBehaviorException(
                    "Player.hasWeapon is returning false for a weapon that he should have (either FIRST or one that was added)");
        }
        if (p.hasWeapon(Weapon.PISTOL) || p.hasWeapon(Weapon.SHOTGUN)) {
            throw new IncorrectBehaviorException(
                    "Player.hasWeapon is returning true for a weapon that he should NOT have");
        }
    }
    ...
    public void testPlayerCompareTo() {
        ...
        if (p.compareTo(p2) != 1) {
            throw new IncorrectBehaviorException(
                    "Player.compareTo is not returning 1 when the player on which it is being called has a greater weapon");
        }
        ...
        if (p.compareTo(p2) != -1) {
            throw new IncorrectBehaviorException(
                    "Player.compareTo is not returning -1 when the player on which it is being called has less ammunition");
        }
        ...
        if (p.compareTo(p2) != 1) {
            throw new IncorrectBehaviorException(
                    "Player.compareTo is not returning 1 when the player on which it is being called has more health");
        }
    }
    ...
    public void testChangeAmmo() {
        ...
        if (p.getAmmunitionRoundsForWeapon(Weapon.CHAINSAW) != 15) {
            throw new IncorrectBehaviorException(
                    "Player.changeAmmunitionRoundsForWeapon did not result in ammunition count changing as expected");
        }
    }
    ...
    public void testReduceAmmo() {
        ...
        if (p.getAmmunitionRoundsForWeapon(Weapon.CHAINSAW) != 2) {
            throw new IncorrectBehaviorException(
                    "Player.changeAmmunitionRoundsForWeapon did not result in ammunition count beign reduced as expected");
        }
    }
    ...
    public void testReducePlayerHealth() {
        ...
        if (p.getHealth() != 50) {
            throw new IncorrectBehaviorException(
                    "Player.changeHealth, with a negative number, did not result in health changing as expected");
        }
    }
    ...
    public void testAddAmmo() {
        ...
        if (p.getAmmunitionRoundsForWeapon(Weapon.PISTOL) != 10) {
            throw new IncorrectBehaviorException(
                    "Player.addAmmunition did not result in ammunition count changing as expected");
        }
    }
    ...
    public void testMonsterCompareTo() {
        ...
        if (compare != 0) {
            throw new IncorrectBehaviorException(
                    "Monster.compareTo should return 0 when compared to itself, but returned " + compare);
        }
        compare = demon.compareTo(baron);
        if (compare != 1) {
            throw new IncorrectBehaviorException(
                    "Monster.compareTo should return 1 when compared to its protector, but instead returned "
                            + compare);
        }
        compare = baron.compareTo(demon);
        if (compare != -1) {
            throw new IncorrectBehaviorException(
                    "Monster.compareTo should return -1 when compared to a monster it protects, but instead returned "
                            + compare);
        }
        compare = spectre.compareTo(imp);
        if (compare != 1) {
            throw new IncorrectBehaviorException(
                    "Monster.compareTo should return 1 when compared to a monster with a lower ordinal, but instead returned "
                            + compare);
        }
        compare = imp.compareTo(demon);
        if (compare != -1) {
            throw new IncorrectBehaviorException(
                    "Monster.compareTo should return -1 when compared to a monster with a higher ordinal, but instead returned "
                            + compare);
        }
        ...
        if (compare != shouldReturn) {
            throw new IncorrectBehaviorException(
                    "Monster.compareTo should be based on hashCode when comparing two monsters of the same type. Should've returned "
                            + shouldReturn + " but instead returned " + compare);
        }
        shouldReturn *= -1;
        compare = imp2.compareTo(imp);
        if (compare != shouldReturn) {
            throw new IncorrectBehaviorException(
                    "(2) Monster.compareTo should be based on hashCode when comparing two monsters of the same type. Should've returned "
                            + shouldReturn + " but instead returned " + compare);
        }
    }
    ...
    public void testGetDefaultProtector() {
        ...
        if (m.getProtectedBy() != m.getMonsterType().getProtectedBy()) {
            throw new IncorrectBehaviorException(
                    "Monster with no custom protector did not return the protector of its monster type");
        }
    }
    ...
    public void testAttackWithNoAmmo() {
        ...
        try {
            ...
            throw new IncorrectBehaviorException(
                    "When attacked with a insufficient ammunition, a BARON_OF_HELL should throw an IllegalArgumentException");
        } catch (IllegalArgumentException e) {
        }
    }
    ...
    public void testAttackDeadMonster() {
        ...
        try {
            ...
            m.attack(m.getMonsterType().weaponNeededToKill, 1);
            throw new IncorrectBehaviorException(
                    "Monster.attack did not throw an IllegalStateException when the monster was attacked when it was already dead");
        } catch (IllegalStateException e) {
        }
    }
